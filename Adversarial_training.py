...
discriminator = discriminator.RNNModel(args.model, ntokens, args.emsize, args.nhid,
                       args.nlayers, args.nclass, args.embedding, args.dropout_em, 
...
def dis_pre_train_step():
    ...
    lab_token_seqs = torch.from_numpy(np.transpose(lab_token_seqs)).cuda(env_settings.CUDA_DEVICE)
    labels = torch.from_numpy(np.transpose(labels)).cuda(env_settings.CUDA_DEVICE)
...
def adv_train_step(judge_only=True):
    ...
    unl_token_seqs = torch.from_numpy(np.transpose(unl_token_seqs)).cuda(env_settings.CUDA_DEVICE)
...
def evaluate():
    ...
    with torch.no_grad():
        for i_batch, sample_batched in enumerate(test_loader):
            token_seqs = torch.from_numpy(np.transpose(sample_batched[0])).cuda(env_settings.CUDA_DEVICE)
            labels = torch.from_numpy(np.transpose(sample_batched[3])).cuda(env_settings.CUDA_DEVICE)
